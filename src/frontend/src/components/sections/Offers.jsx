...
export default function PopularSection() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch("http://localhost:3000/api/products/filter/offer");
        if (!res.ok) throw new Error("Error al obtener productos en ofertas");
  ...
  return (
    <div className="my-12 px-4">
      <h2 className="text-4xl font-bold text-center mb-2 text-primary">¡Ofertas!</h2>
      <p className="text-center text-2xl text-testSecondary mb-8">Aproveha nuestras mejores ofertas</p>
